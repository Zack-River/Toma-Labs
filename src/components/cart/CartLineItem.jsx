...
export function CartLineItem({ item, onChangeQuantity, onRemove }) {
  ...
  return <article className="cart-line">
    {catalogItem ? <CatalogLink className="cart-thumb" item={catalogItem} ariaLabel={`View ${item.name}`}><img src={item.image || '/assets/toma-turbo-mode.jpg'} alt="" /></CatalogLink> : <div className="cart-thumb custom"><img src="/assets/toma-bean-mark-128.png" alt="" /><strong>toma</strong><span>BLEND LAB</span></div>}
    ...
    <div className="cart-price"><strong>{priceValue}</strong><small>{item.priceKnown ? 'Unit price' : 'Quote required before checkout'}</small><div className="quantity-control"><button type="button" onClick={() => onChangeQuantity(item.id, item.quantity - 1)} aria-label={`Decrease quantity of ${item.name}`}><TomaIcon icon={faMinus} /></button><output aria-live="polite">{item.quantity}</output><button type="button" onClick={() => onChangeQuantity(item.id, item.quantity + 1)} aria-label={`Increase quantity of ${item.name}`}><TomaIcon icon={faPlus} /></button></div><button className="remove-line" type="button" onClick={() => onRemove(item.id)} aria-label={`Remove ${item.name} from bag`}>Remove</button></div>
  </article>;
}
