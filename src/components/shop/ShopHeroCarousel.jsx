...
export function ShopHeroCarousel({ slides }) {
  ...
  return <section ref={carouselRef} className="shop-hero" aria-roledescription="carousel" aria-label="TOMA collection highlights" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false); }}>
    <div className="shop-hero-stage" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} onPointerCancel={() => { dragStart.current = null; }}>{slides.map((slide, index) => <article className={`shop-hero-slide shop-hero-slide-${slide.id}${index === activeIndex ? ' active' : ''}`} key={slide.id} aria-hidden={index !== activeIndex} aria-roledescription="slide" aria-label={`${index + 1} of ${slides.length}`}>
      <div className="shop-hero-copy"><p className="shop-eyebrow">{slide.eyebrow}</p><h1>{slide.titleLine}<br /><em>{slide.titleEmphasis}</em></h1><p className="shop-hero-lede">{slide.copy}</p><div className="shop-hero-actions"><Link className="button button-brass" to={slide.ctaTarget} tabIndex={index === activeIndex ? 0 : -1}>{slide.ctaLabel} <span aria-hidden="true">↗</span></Link><Link className="button button-line" to={slide.secondaryTarget} tabIndex={index === activeIndex ? 0 : -1}>{slide.secondaryLabel} <span aria-hidden="true">↘</span></Link></div><div className="shop-hero-proof">{slide.proof.map((item, proofIndex) => <span key={item}>{proofIndex === 0 ? <b>{item.split(' ')[0]}</b> : null}{proofIndex === 0 ? ` ${item.split(' ').slice(1).join(' ')}` : item}</span>)}</div></div><div className="shop-hero-visual"><div className="shop-hero-image" style={{ backgroundImage: `linear-gradient(90deg, rgba(36, 21, 13, .86), rgba(36, 21, 13, .18)), url('${slide.image}')` }}></div><div className="shop-hero-label"><img src="/assets/toma-bean-mark-128.png" width="128" height="128" alt="" /><div><strong>toma</strong><small>{slide.label}</small><em>SELECTED FOR YOUR NEXT CUP</em></div></div><span className="shop-hero-note">{slide.note}</span><span className="shop-hero-index">DROP 0{index + 1} <b aria-hidden="true">↘</b></span></div>
    </article>)}</div>
    <div className="shop-carousel-controls"><button type="button" onClick={() => goTo(activeIndex - 1)} aria-label="Previous shop highlight">←</button><div className="shop-carousel-dots">{slides.map((slide, index) => <button className={index === activeIndex ? 'active' : ''} type="button" key={slide.id} onClick={() => goTo(index)} aria-label={`Show slide ${index + 1}: ${slide.label}`} aria-current={index === activeIndex ? 'true' : undefined}><span></span></button>)}</div><button type="button" onClick={() => goTo(activeIndex + 1)} aria-label="Next shop highlight">→</button><span className="shop-carousel-count">0{activeIndex + 1} / 0{slides.length}</span></div>
  </section>;
}
