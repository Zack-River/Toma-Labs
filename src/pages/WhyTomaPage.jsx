...
export function WhyTomaPage() {
  return <div className="route-page toma-destination-page why-toma-page">
    ...
    <main id="main-content">
      ...
      <section className="why-standard-section" aria-labelledby="standard-title"><div className="why-standard-art"><img src="/assets/toma-turbo-editorial.png" alt="TOMA coffee package and prepared cup" /><span className="why-standard-label">THE TOMA<br />STANDARD / 01</span></div><div className="why-standard-copy"><p className="destination-eyebrow">From experience to shelf</p><h2 id="standard-title">Every choice should<br /><em>earn its place.</em></h2><p>We keep the catalogue legible: what it is, how it tastes, what format it comes in, and why it might belong in your routine. When the ready-made shelf is not enough, the Blend Lab explains the next move instead of leaving you alone with ingredients.</p><div className="why-standard-list">{STANDARD_STEPS.map((step) => <div key={step.number}><span>{step.number}</span><div><h3>{step.title}</h3><p>{step.copy}</p></div></div>)}</div><Link className="text-link" to="/blend-lab">See the Blend Lab <span aria-hidden="true">↗</span></Link></div></section>
