    ...
    <main id="main-content">
      <section className="destination-hero finder-hero">
        <div className="destination-hero-copy">
          ...
          <div className="destination-actions"><Link className="destination-button destination-button-brass" to="#coffee-finder">Start with a moment <span aria-hidden="true">↓</span></Link><Link className="destination-button destination-button-line" to="/blend-lab">Make a custom blend <span aria-hidden="true">↗</span></Link></div>
        </div>
      ...
      <section className="coffee-finder-section" id="coffee-finder" aria-labelledby="finder-title">
        ...
        <div className="finder-choice-grid" role="group" aria-label="Coffee moments">
          {GOALS.map((item) => <button className={`finder-choice${item.id === goal.id ? ' is-selected' : ''}`} type="button" key={item.id} aria-pressed={item.id === goal.id} onClick={() => setSelectedGoalId(item.id)}><span>{item.number}</span><strong>{item.label}</strong><small>{item.shortLabel}</small><i aria-hidden="true">↗</i></button>)}
        </div>
        <article className="finder-result" aria-live="polite">
          <div className="finder-result-copy"><p className="destination-eyebrow">{goal.number} / TOMA starting point</p><h2>{goal.message}</h2><p>{goal.recommendation}</p><p className="finder-result-note"><strong>Recommended direction</strong><CatalogLink item={recommendation}>{recommendation?.name}</CatalogLink> · {recommendation?.priceLabel} · {recommendation?.subtitle}</p><div className="destination-actions"><button className="destination-button destination-button-brass" type="button" onClick={addRecommendation}>Add {recommendation?.name} <span aria-hidden="true">↗</span></button><Link className="destination-button destination-button-line-dark" to="/blend-lab">Customize this direction <span aria-hidden="true">↗</span></Link></div></div>
